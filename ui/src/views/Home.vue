<template>
  <div style="padding:5px;">
    <el-button style="margin-right: 4px;" type="primary" round icon="Refresh" @click="handleRefresh" :disabled="loading">{{ $t('Refresh List') }}</el-button>
    <el-input style="margin-right: 4px;width:200px" v-model="filterString" search @input="handleSearch" :placeholder="$t('Please enter the filter key...')"/>
    ...
    <div style="float: right; margin-right: 50px">
      <span style="margin-right: 20px; color: var(--el-color-primary); font-size: 24px">{{ $t('device-count', {count: devlists.filter(dev => dev.online).length}) }}</span>
    ...
    <el-table :loading="loading" :data="filteredDevices" style="margin-top: 10px; width: 100%" :empty-text="$t('No devices connected')" @selection-change='handleSelection'>
      ...
      <el-table-column width="200">
        <template #default="{ row }">
          ...
          <el-tooltip v-if="row.online" placement="top" :content="$t('Access your device\'s Shell')">
            <i class="iconfont icon-shell" style="font-size: 28px; color: black; cursor:pointer;" @click="connectDevice(row.id)"/>
          </el-tooltip>
          <el-tooltip v-if="row.online" placement="top" :content="$t('Access your devices\'s Web')">
            <i class="iconfont icon-web" style="font-size: 28px; color: #409EFF; cursor:pointer;" @click="connectDeviceWeb(row)"/>
          </el-tooltip>
    ...
    <el-dialog v-model="cmdStatus.respModal" :title="$t('Response of executive command')" :width="800">
      <el-table :data="cmdStatus.responses" :empty-text="$t('No Response')">
        ...
        <el-table-column type="index" label="#" width="100" />
        <el-table-column prop="id" :label="$t('Device ID')" width="200" />
        <el-table-column prop="code" :label="$t('Status Code')" width="200" />
        <el-table-column prop="err" :label="$t('Error Code')" width="200" />
